  ...
  public function index() {
    if( isset($this->request->post['ci']) && isset($this->request->post['bi']) && strlen($this->request->post['bi']) == 10 ) {
      ...
      if( is_numeric($insertStatus) ) {
        ...
        $output = array( 'status' => false, 'error' => 'Not authorized to update seen!' );
      } else {
        $output = array('status' => false, 'error' => 'Already Seen!');
      }
    } else {
      $output = array('status' => false, 'error' => 'Wrong board selected!');
    }
