        ...
        if( $board_detail['board_admin'] == $this->user->user_id || in_array($this->user->user_id, $admins) ) {
          ...
          if( !isset($output) ) {
            ...
            if( is_numeric($notice_cr_status) ) {
              ...
              $output = array( 'status' => false, 'error' => 'Can\'t create notice!' );
            }
          }

        } else $output = array( 'status' => false, 'error' => 'error' );
      } else {
        $output = array( 'status' => false, 'error' => 'It seems like you have selected Incorrect board' );
      }
    } else {
      $output = array( 'status' => false, 'error' => 'You are not allowed to send notice to this board' );
